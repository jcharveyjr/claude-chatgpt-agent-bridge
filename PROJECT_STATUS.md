...
| Add task-store and log retention or rotation | High | Task-store retention implemented (July 17) in PR; pending ChatGPT verification. Log rotation still to do | Claude | ChatGPT verifies behavior; Codex reviews later | None | Runtime data cannot grow indefinitely and retention behavior is documented |
| Add a status command for health, PID, clients, queue, and failures | High | Implemented (July 17) in PR; pending ChatGPT live Windows verification | Claude | ChatGPT performs live Windows verification | None | One command reports actionable bridge state |
